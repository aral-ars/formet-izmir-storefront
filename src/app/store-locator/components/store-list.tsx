              ...
              {stores.map((store) => {
                ...
                return (
                  <SidebarMenuItem key={store.id}>
                    <SidebarMenuButton isActive={active} className="h-auto flex-col items-stretch gap-0 p-3" render={<button type="button" ref={(el) => {
                                                  if (el) itemRefs.current.set(store.id, el);
                                                  else itemRefs.current.delete(store.id);
                                                }} onClick={() => onSelect(store.id)} aria-current={active} />}><div className="flex items-center justify-between gap-2">
                                                ...
                                                </div><div className="text-muted-foreground mt-2 space-y-1.5 text-xs font-normal tabular-nums">
